...
# --- LLGC Training Helper Function (Modified for Unsupervised Embedding) ---

def train(model, train_features, dummy_labels, epochs, lr):
    """
    Modified training function for unsupervised embedding.
    It uses a dummy cross-entropy loss with zero-labels to train the 
    LorentzLinear layer's projection weights, forcing the model to learn a 
    meaningful low-dimensional hyperbolic representation (Z_t).
    """
    optimizer = optim.Adam(model.parameters(), lr=lr)
    t = perf_counter()
    # Using a reduced number of epochs is typical for pre-training/embedding
    for epoch in range(epochs):
        model.train()
        optimizer.zero_grad()
        
        # model(train_features) outputs the Lorentzian embedding Z_t
        output = model(train_features)
        
        # Loss uses the dummy label tensor (all zeros)
...
def run_anomaly_detection(embeddings):
    """
    Fits an IsolationForest model on the learned embeddings to identify anomalies.
    """
    # Initialize Isolation Forest. contamination='auto' estimates the contamination
    clf = IsolationForest(contamination='auto', random_state=42)
    
    # Isolation Forest is fitted and predicts outliers
    clf.fit(embeddings)
    anomalies = clf.predict(embeddings) # -1 is anomaly, 1 is inlier
    
    # Get the decision function (anomaly score: lower is more anomalous)
...
# LLGC Propagation Hyperparameters (from LLGC-main/run_cora.py defaults)
K_PROP = 20           # K: number of propagation steps (PageRankAgg).
ALPHA = 0.1         # alpha: PageRankAgg alpha
DROP_OUT = 0.0      # Hyperbolic linear drop_out
USE_BIAS = 1        # Hyperbolic linear bias (1 or 0)

# LLGC Dummy Training Hyperparameters
EPOCHS = 100        # Number of epochs for dummy training.
LR = 0.6            # Initial learning rate.

# Anomaly Detection Parameters
# The desired output dimension of the LLGC model (Z_t)
EMBEDDING_DIM = 64 
# The target size for the dummy label tensor for F.cross_entropy
DUMMY_N_CLASSES = 2 


# --- Anomaly Detection Execution Loop ---

# The following helper functions are defined in your preceding code block 
# and are used here: sparse_to_torch_sparse, normalize_adj_sym, adj_matrix, X_tensor, temporal_segments.

print("\n" + "="*70)
print(f"Step 9: L²GC Embedding and Anomaly Detection on Temporal Subgraphs")
print(f"Embedding Dimension: {EMBEDDING_DIM}, Training Device: {DEVICE}")
print("="*70)
...
for (t_start, t_end), global_indices in temporal_segments.items():
    ...
    # Extract the sub-adjacency matrix for the current nodes
    adj_segment_unnorm = adj_matrix[global_indices_np[:, None], global_indices_np]
    
    # Re-normalize the sub-adjacency matrix
    ...
    # This step propagates features in the Euclidean space (like SGC)
    ...
    # X_gconv contains the propagated features (input to the LLGC layer)
    ...
    # Initialize LLGC model with EMBEDDING_DIM as the number of classes (output size)
    model = LLGC(in_feat, EMBEDDING_DIM, DROP_OUT, USE_BIAS).to(DEVICE)

    # Create DUMMY labels: a tensor of zeros, size N_t, required for cross_entropy loss.
    dummy_labels = torch.zeros(N_t).long().to(DEVICE)
    
    # Train the model to learn the hyperbolic projection (Lorentzian linear layer)
    model_trained, train_time = train(model, X_gconv, dummy_labels, EPOCHS, LR)
    
    # --- 5. Extract Final Embeddings (Z_t) ---
    # The forward pass performs: Euclidean -> Hyperbolic -> Tangent Space (Euclidean Output)
    model_trained.eval()
    # Z_t is the final Euclidean embedding, detached and moved to NumPy/CPU
    Z_t = model_trained(X_gconv).cpu().detach().numpy() 
    ...
    anomaly_results[(t_start, t_end)] = {
        ...
        'embeddings': Z_t, # Store the embeddings for further analysis
        'indices': global_indices # Store indices to map back to original data
    }
